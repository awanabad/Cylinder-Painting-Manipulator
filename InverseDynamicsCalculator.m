function Tao=InverseDynamicsCalculator(q_in,qd_in,qdd_in) 
%Function that calculates Torques based on the Euler Lagrange Method
%************************************************************************
...
Dxqdd_test=subs(subs(subs(Term1,q,q_in),qd,qd_in),qdd,qdd_in)

Term2=simplify(((([C1*qd,C2*qd,C3*qd].')*qd)));
Cxqd_test=subs(subs(subs(Term2,q,q_in),qd,qd_in),qdd,qdd_in)

Term3=G;
G_test=subs(G,q,q_in) 
%Summation and Simplification of Tao Terms 
TaoTemp=simplify(Term1+Term2+Term3); 

%Substitution of input values 
Tao_subs=subs(TaoTemp,q,q_in);%subbing in positions
Tao_subs2=subs(Tao_subs,qd,qd_in); %subbing in velocities
Tao_subs3=subs(Tao_subs2,qdd,qdd_in); %subbing in accelerations
